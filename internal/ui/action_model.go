    ...
    case tea.KeyMsg:
        ...
        case "tab":
            ...
            if focusedKey != "" {
                ...
                // Blur list by setting no selection (list uses cursor) and focus first text input.
                // Note: list.Cursor() always returns index; there is no blur; we simply focus text input and leave list selection unchanged.
                // Focus first input.
...
func (m actionModel) View() string {
    ...
    s := fmt.Sprintf("%s • Tool: %s  (←/→ switch)\n", m.action.Title, m.tools[m.toolIdx])
    s += "TAB to switch fields • ENTER to toggle/confirm • +/- to adjust • ESC to cancel\n\n"
